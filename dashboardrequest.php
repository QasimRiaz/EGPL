    ...
    try {
        ...
        foreach ($flat as $index => $taskdate) {
             ...
             if($statscolcount > 35 ){
                ...
                $get_taskstatus_pending = array(
                    'role__not_in' => 'Administrator',
                    'meta_query' => array(
                        'relation' => 'OR',
                        array(
                            'key' => $index.'_status',
                            'value' => 'Pending', // date to compare to, after this one
                            'compare' => '=',
                        ),
                        array(
                            'key' => $index.'_status',
                ...
                $taskstatusdata_pending = new WP_User_Query( $get_taskstatus_pending );
                $thistask_pending_total = $taskstatusdata_pending->get_results();
                ...
                $task_pending_total_task_array['data'][] = count($thistask_pending_total);
...
function getdashboardactiveusergraph() {
    ...
    try {
        ...
        foreach ($user_pie_chart_stats['avail_roles'] as $rolename=>$rolecount){
            if($rolename != 'administrator' && !empty($rolename)){
